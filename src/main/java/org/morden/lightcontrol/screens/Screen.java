...
public class Screen {
    ...
    private Screen(ScreenDescription ds) {
        ...
        memory = new int[ds.addrWidth][ds.addrHeight];        
    }
    ...
    public void setPixel(int x, int y, int data, boolean checkMemory) {
        ...
        if (!checkMemory || memory[x][y]!=data) {
            for (Location l : pixel) {
                Block b = l.getBlock();
                ds.blockUpdater.UpdateBlock(b, data);
            }
            memory[x][y] = data;
        } 
    }
    ...
    public static Screen generateScreen(Location l0, Location l1, Material onmaterial, Material offmaterial, BlockUpdater blockupdater) {
        ScreenDescription ds = scanScreen(l0, l1, onmaterial, offmaterial, blockupdater);        
    ...
    public static Screen generateScreen(Location l0, Location l1, int addrWidth, int addrHeight, Material onmaterial, Material offmaterial, BlockUpdater blockupdater) {
        ScreenDescription ds = scanScreen(l0, l1, onmaterial, offmaterial, blockupdater);        
    ...
    public static ScreenDescription scanScreen(Location l0, Location l1, Material onmaterial, Material offmaterial, BlockUpdater blockupdater) {        
        ...
        if (dx==0 && dy!=0 && dz!=0) { // zy plane
            ...
            if (world.getBlockTypeIdAt(x1+1, y1, z1)==onmaterial.getId() || world.getBlockTypeIdAt(x1+1, y1, z1)==offmaterial.getId())
                origin = new Location(world, x1+1, y1, z1);
            else if (world.getBlockTypeIdAt(x1-1, y1, z1)==onmaterial.getId() || world.getBlockTypeIdAt(x1-1, y1, z1)==offmaterial.getId())
                origin = new Location(world, x1-1, y1, z1);
            else throw new IllegalArgumentException("Can't find origin block.");
            ...
            if (world.getBlockTypeIdAt(x1, y1+1, z1)==onmaterial.getId() || world.getBlockTypeIdAt(x1, y1+1, z1)==offmaterial.getId())
                origin = new Location(world, x1, y1+1, z1);
            else if (world.getBlockTypeIdAt(x1, y1-1, z1)==onmaterial.getId() || world.getBlockTypeIdAt(x1, y1-1, z1)==offmaterial.getId())
                origin = new Location(world, x1, y1-1, z1);
            else throw new IllegalArgumentException("Can't find origin block.");
            ...
            if (world.getBlockTypeIdAt(x1, y1, z1+1)==onmaterial.getId() || world.getBlockTypeIdAt(x1, y1, z1+1)==offmaterial.getId())
                origin = new Location(world, x1, y1, z1+1);
            else if (world.getBlockTypeIdAt(x1, y1, z1-1)==onmaterial.getId() || world.getBlockTypeIdAt(x1, y1, z1-1)==offmaterial.getId())
                origin = new Location(world, x1, y1, z1-1);
            else throw new IllegalArgumentException("Can't find origin block.");
        ...
        ds.onMaterial = onmaterial;
        ds.offMaterial = offmaterial;
        ds.blockUpdater = blockupdater;
        return ds;
    }
